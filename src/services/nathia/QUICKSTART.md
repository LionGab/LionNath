...
const mensagem = 'Não aguento mais essa situação';
...
const comentario = 'Você deveria fazer X...';
...
switch (acao) {
  case 'approve':
    ...
    break;
  case 'reject':
    mostrarFeedback('Comentário contém julgamento');
    break;
  case 'review':
...
if (health.status !== 'healthy') {
  console.error('NAT-IA não está saudável:', health);
  // Alertar equipe
}
...
try {
  ...
  if (error instanceof ValidationError) {
    // Erro de validação - mostrar para usuária
...
describe('Chat', () => {
  it('deve processar mensagem', async () => {
    ...
    const res = await chatEmpatico('Olá', ctx);
...
console.log('Thresholds moderação:', NATHIA_CONFIG.moderacao);
...
6. **recomendacoes** - Conteúdo + grupos + hábitos
7. **habitos** - Micro-objetivos + motivação
8. **analytics** - Rótulos + métricas (sem PII)
